...
# Создаём таблицы (для SQLite без миграций)
...
# Главная страница (HTML как было «сайт»)
@app.get("/", response_class=HTMLResponse)
async def index(request: Request):
    # имя шаблона подставь то, которое у тебя реально есть, например "index.html"
    return templates.TemplateResponse("index.html", {"request": request})


# Auth endpoint для получения токена
